...
export function ModulesTab({ canUseDb, checkingUpdatePrerequisites, chooseDatabase, createJob, loading, loadingModules, moduleFilters, modules, moduleSelectionBlock, odooDatabases, openRepositoryImport, openSocleDialog, openZipImport, overview, projectHeaderHeight, projectTabsHeight, refinedInterface, refreshModules, requestDeleteCode, requestTranslationReset, requestUninstall, requestUpdateAllOdooModules, selectedDb, selectedModules, selectedProject, selectedProjectReady, setSelectedModules, settings, stickyHeader }: ModulesTabProps) {
  ...
  const moduleFiltersBlock = (
    <div className="grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_180px_210px_220px]">
      <div className="relative">
        ...
        <Input className="pl-9" placeholder="Rechercher par nom de module" value={moduleFilters.search} onChange={(event) => moduleFilters.setSearch(event.target.value)} />
      </div>
  ...
  const modulePaginationBlock = moduleFilters.filtered.length > 0 && (
    <div className="flex flex-col gap-3 border-t bg-muted/30 px-3 py-2 text-sm sm:flex-row sm:items-center sm:justify-between">
      <span className="text-muted-foreground">
        {Math.min((moduleFilters.page - 1) * MODULES_PER_PAGE + 1, moduleFilters.filtered.length)}–{Math.min(moduleFilters.page * MODULES_PER_PAGE, moduleFilters.filtered.length)} sur {moduleFilters.filtered.length} module(s)
      </span>
      <div className="flex items-center gap-2">
        <Button size="icon" variant="outline" disabled={moduleFilters.page <= 1} onClick={() => moduleFilters.setPage((page) => Math.max(1, page - 1))} aria-label="Page précédente" title="Page précédente">
          <ChevronLeft className="h-4 w-4" />
        </Button>
        <span className="min-w-20 text-center tabular-nums">Page {moduleFilters.page}/{moduleFilters.pageCount}</span>
        <Button size="icon" variant="outline" disabled={moduleFilters.page >= moduleFilters.pageCount} onClick={() => moduleFilters.setPage((page) => Math.min(moduleFilters.pageCount, page + 1))} aria-label="Page suivante" title="Page suivante">
      ...
      <Card>
        <CardHeader className="gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="grid w-full gap-2 sm:w-auto sm:grid-cols-2">
            ...
            <Button className="w-full" variant="outline" onClick={openRepositoryImport} disabled={!selectedProjectReady || loading}>
              <CloudDownload className="h-4 w-4" />
              Dépôt SSH · Ajout / MAJ
            </Button>
