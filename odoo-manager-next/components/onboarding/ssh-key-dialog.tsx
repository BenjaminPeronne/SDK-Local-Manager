...
export function SshKeyDialog({
  ...
  setSshComment,
  setSshKeyBackup,
  setSshRegenerateConfirmed,
  setSshRegenerateMode,
  sshComment,
  sshKeyBackup,
  sshKeys,
  sshRegenerateConfirmed,
  sshRegenerateMode,
  ...
  async function requestSshKeyGeneration(replace = false) {
    ...
    try {
      const key = await api<SshPublicKey & { created: boolean; message: string; backup?: string }>(
        ...
        {sshKeys.length === 0 ? (
          <div className="space-y-4">
            <div className="grid gap-1.5">
              <label className="text-sm font-medium" htmlFor="ssh-key-comment">
                E-mail professionnel ou commentaire
              </label>
              <Input
                id="ssh-key-comment"
                value={sshComment}
          ...
          <div className="space-y-4">
            <div className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-800 dark:bg-amber-950/45 dark:text-amber-100">
              ...
              <ul className="mt-2 list-disc space-y-1 pl-4 text-xs leading-5">
                <li>
                  Une nouvelle paire de clés remplace <code>~/.ssh/id_ed25519</code> sur cette machine.
                </li>
                <li>
            ...
            <div className="grid gap-1.5">
              <label className="text-sm font-medium" htmlFor="ssh-key-regenerate-comment">
                E-mail professionnel ou commentaire
              </label>
              <Input
          ...
          <div className="space-y-4">
            ...
            {sshKeys.length > 1 && (
              <div className="grid gap-1.5">
                <label className="text-sm font-medium" htmlFor="ssh-public-key-select">
                  Clé publique
                </label>
                <Select value={selectedSshKey?.name || ""} onValueChange={setSelectedSshKeyName}>
                  <SelectTrigger id="ssh-public-key-select">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {sshKeys.map((key) => (
